...
module.exports = class es extends Exchange {
    describe () {
    ...
    async fetchMarkets (params = {}) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if ('PRICE_FILTER' in filters) {
                ...
                entry['precision']['price'] = filter['display'] || filter['tick_size'];
            }
  ...
  var type = this.safeString (order, 'type');
        var side = this.safeString (order, 'side');
        type = type ? type.toLowerCase() : type;
        side = side ? side.toLowerCase() : side;
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            const extobject = path !== 'leverage' ? {
                'timestamp': Date.now(),
                'recv_window': this.options['recvWindow'],
            } : {};
            const query = this.urlencode (this.extend (extobject, params));
